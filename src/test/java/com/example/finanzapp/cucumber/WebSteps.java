...
public class WebSteps {
  ...
  @Given("I have chart data with debit marker for {string}")
  public void iHaveChartDataWithDebitMarkerFor(String email) {
    User owner = ensureUserExists(email);
    owner.setLanguage("EN");
    userRepository.save(owner);

    balanceDailyRepository.deleteByUser(owner);
    transactionRepository.deleteByUser(owner);

    LocalDate start = LocalDate.now().minusDays(29);
    for (int i = 0; i < 30; i++) {
      BalanceDaily point = new BalanceDaily();
      point.setUser(owner);
      point.setDate(start.plusDays(i));
      point.setBalanceCentsEndOfDay(100_00L + i * 50L);
      balanceDailyRepository.save(point);
    }

    Transaction debit = new Transaction();
    debit.setUser(owner);
    debit.setBookingDateTime(LocalDateTime.now().withHour(10).withMinute(15).withSecond(0).withNano(0));
