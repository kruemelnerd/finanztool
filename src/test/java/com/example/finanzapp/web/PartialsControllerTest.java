...
class PartialsControllerTest {
  ...
  @Test
  void balanceChartRendersRetryStateForInvalidRange() throws Exception {
    User chartUser = userRepository.findByEmail("invalid-range-user@example.com").orElseGet(() -> {
      User created = new User();
      created.setEmail("invalid-range-user@example.com");
      created.setPasswordHash("hashed");
      created.setLanguage("EN");
      return userRepository.save(created);
    });
    chartUser.setLanguage("EN");
    userRepository.save(chartUser);

    mockMvc.perform(get("/partials/balance-chart?range=invalid").with(user("invalid-range-user@example.com")))
        .andExpect(status().isOk())
        .andExpect(content().string(containsString("Chart could not be loaded right now. Please try again.")))
